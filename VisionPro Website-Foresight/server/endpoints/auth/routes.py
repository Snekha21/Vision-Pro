...
class Register(Resource):
    ...
    def post(self):
        ...
        if  email_exist:
            ...
            return {"success": False, "msg": "This email already exists."}, 201
        ...
        if not role:
            return {'error': f'Role "{role}" not found'}, 201


        # Check password strength
        if is_strong_password(_passwd)==False:
       
            return {"success": False, "msg": "strong password must have at least one uppercase letter (A-Z), at least one lowercase letter (a-z), at least one digit (0-9), at least one special character from the set: !@#$%^&*, and a minimum length of 8 characters."}, 201
